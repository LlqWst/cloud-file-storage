...
import dev.lqwd.cloudfilestorage.infrastructure.mapper.DownloadResponseMapper;
...
public class DownloadService {
    ...
    private final DownloadResponseMapper mapper;
    ...
    public DownloadedResponseDto download(String rawPath, long id) {
        ...
        if (isDirectory(path)) {
            return mapper.toDirResponseDto(path, getZipBytes(id, requestedPath, path.resourceName()));
        }
        return mapper.toFileResponseDto(path, getFileBytes(id, requestedPath));
    }
    ...
    private StreamingResponseBody getZipBytes(long id, String requestedPath, String baseFolder) {
        return outputStream -> {
            try (ZipOutputStream zipOut = new ZipOutputStream(outputStream)) {
                findStorageService.findDirResourcesNameWithoutDirRecursive(requestedPath, id)
                        .forEach(resourceName -> processResource(zipOut, resourceName, baseFolder));
            } catch (Exception e) {
    ...
    private void processResource(ZipOutputStream zipOut, String resourceName, String baseFolder) {
        ...
        try {
            ...
            throw new InternalErrorException("Error during directory streaming for resource " + resourceName, e);
        }
    }
